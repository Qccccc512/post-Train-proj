...
if [[ -z "${RUN_NAME}" ]]; then
  ...
  echo "Usage: bash launchers/global/run_benchmark.sh <repo_root> <run_name>" >&2
...
if [[ "${DRY_RUN}" != "1" && ( "${needs_fetch}" == "1" || "${USE_REMOTE_MERGED_MODELS}" != "1" ) ]]; then
  ...
  elif [[ "${SKIP_SETUP}" != "1" ]] && ! "${TRAIN_PYTHON_BIN}" -c "import unsloth" >/dev/null 2>&1; then
    echo "Training/export runtime is missing Unsloth; preparing the current Colab environment."
    run_cmd bash launchers/global/setup_train_env.sh "${REPO_ROOT}"
  fi
fi
...
if [[ "${needs_fetch}" == "1" ]]; then
  ...
  echo "========================================"
  run_train_python scripts/hub/hf_repo_sync.py fetch-run --run-name "${RUN_NAME}"
fi
...
if [[ "${USE_REMOTE_MERGED_MODELS}" == "1" ]]; then
  ...
  echo "========================================"
  EXPORT_CMD=(
    scripts/train/export_merged_model_variants.py
...
if [[ "${DRY_RUN}" != "1" ]]; then
  ...
  if [[ "${BENCHMARK_RUNTIME_MODE_RESOLVED}" == "conda" ]]; then
    if [[ "${SKIP_SETUP}" != "1" ]] && ( ! contains_conda_env "${BENCHMARK_ENV_NAME}" || [[ "${FORCE_BENCHMARK_SETUP}" == "1" ]] ); then
      echo "========================================"
      echo "3. Preparing Benchmark Runtime"
      echo "========================================"
      run_cmd bash launchers/local/setup_bench_env.sh "${REPO_ROOT}"
    else
      echo "========================================"
    ...
    if [[ "${SKIP_SETUP}" != "1" ]] && ( [[ "${FORCE_BENCHMARK_SETUP}" == "1" ]] || ! "${BENCHMARK_VENV_DIR}/bin/python" -c "import vllm, lm_eval, bitsandbytes" >/dev/null 2>&1 ); then
      echo "========================================"
      echo "3. Preparing Benchmark Runtime"
      echo "========================================"
      BENCHMARK_VENV_DIR="${BENCHMARK_VENV_DIR}" run_cmd bash launchers/global/setup_bench_env.sh "${REPO_ROOT}"
    else
      echo "========================================"
      echo "3. Benchmark Runtime Already Present"
      echo "========================================"
    ...
    if [[ "${SKIP_SETUP}" != "1" ]] && ( [[ "${FORCE_BENCHMARK_SETUP}" == "1" ]] || ! "${BENCHMARK_PYTHON_BIN}" -c "import vllm, lm_eval, bitsandbytes" >/dev/null 2>&1 ); then
      echo "========================================"
      echo "3. Preparing Benchmark Runtime"
      echo "========================================"
      BENCHMARK_SETUP_MODE=global BOOTSTRAP_PYTHON="${BENCHMARK_PYTHON_BIN}" run_cmd bash launchers/global/setup_bench_env.sh "${REPO_ROOT}"
    else
      echo "========================================"
      echo "3. Benchmark Runtime Already Present"
      echo "========================================"
  ...
  echo "========================================"
  if [[ "${BENCHMARK_RUNTIME_MODE_RESOLVED}" == "conda" ]]; then
    echo "+ bash launchers/local/setup_bench_env.sh ${REPO_ROOT}"
  elif [[ "${BENCHMARK_RUNTIME_MODE_RESOLVED}" == "venv" ]]; then
    echo "+ BENCHMARK_VENV_DIR=${BENCHMARK_VENV_DIR} bash launchers/global/setup_bench_env.sh ${REPO_ROOT}"
  else
    echo "+ BENCHMARK_SETUP_MODE=global BOOTSTRAP_PYTHON=${BENCHMARK_PYTHON_BIN} bash launchers/global/setup_bench_env.sh ${REPO_ROOT}"
  fi
fi
...
if [[ "${SKIP_BASE}" != "1" ]]; then
  ...
  run_benchmark_python scripts/bench/run_benchmarks.py \
    --suite all \
...
if (( ${#SUMMARY_RUNS[@]} >= 2 )); then
  ...
  echo "========================================"
  SUMMARY_CMD=(
    scripts/analysis/summarize_benchmark_candidates.py
...
from scripts.common.runtime_utils import OFFICIAL_HF_ENDPOINT, get_hf_token, resolve_hf_repo_id
